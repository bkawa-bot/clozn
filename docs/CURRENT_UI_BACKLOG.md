...
- ✅ **J-lens adapter (shipped 2026-07-09, #115 J2–J3)** — the engine-native `POST /jlens` route + the Python studio seam (`POST /jlens`, `POST /runs/<id>/jlens`) + the Run Inspector **"Disposed to say · J-lens"** panel fill the `provider_type: jacobian_lens` slot `docs/WORKSPACE_LENS.md` reserved, with an unskippable provenance caption (fitted linear lens, a disposition not a verified thought) — the "wrap the scoop" answer to the open-weight J-space work (RACE posture: Anthropic published 2026-07-06; nobody else has it in a product yet, per ROADMAP). Fit today for Qwen2.5-7B only — model-agnostic (fit-per-model, any AR GGUF) is scoped, not shipped. **Causally verifying J-lens readouts against Thread-A forced receipts** (self-report vs J-lens vs receipt, the X1 introspection-receipts idea) is still open.
