...
cd /d "%~dp0"
echo === configure (CLOZE_BUILD_CUDA=ON) ===
...
echo === build + run the sae_topk parity test (kernel vs CPU reference) ===
cmake --build build-cuda --target test_sae_topk
if errorlevel 1 (echo BUILD_FAILED & exit /b 3)
ctest --test-dir build-cuda -R sae_topk --output-on-failure
