...
6. **Engine robustness — CONFIRMED ROOT CAUSE (the #1 production blocker): a client disconnecting mid-SSE-stream crashes the single-worker engine.** Isolated cleanly during Phase 0 validation: non-stream `/v1/completions` is fine; a streaming request whose client aborts mid-stream kills the engine (hard exit, no crash trace). This is exactly the recon's flagged gap — *"no interrupted/cancelled-request path in the server"* — made concrete. The studio's own `_engine_complete_traced` consumes the *full* stream, so ordinary studio chat is safe; but any real-world mid-stream disconnect (closed tab, client timeout, cancel) will crash it. **Phase 4 must:** (a) C++ — catch the broken-pipe on the SSE writer instead of hard-exiting; (b) supervise the engine (auto-restart on death); (c) studio — retry-on-connection-refused so a transient engine bounce doesn't surface as a failed chat.
