...
6. **Engine robustness — TWO confirmed crash modes; THE #1 production blocker (bigger than Phase 0 first implied).** The single-worker engine hard-exits (no crash trace) under two conditions, both surfaced during Phase 0/1 validation:
   - **(i) Sustained sequential load — the serious one.** `research/smoke_engine_substrate.py` reliably kills the engine ~7 generations into a *normal* session (chat → receipts → counterfactual → narrate → replay…), reproduced identically twice. This is ordinary usage — every multi-turn session or receipts sweep hits it — so the engine substrate can't sustain a session yet. The studio sends requests *sequentially* (not concurrently), so it's a leak / context-pool race under back-to-back requests; the exact mechanism needs a debug build. Every feature passes on a *fresh* engine (validated manually) — it is purely a survive-under-load problem, not a feature bug.
