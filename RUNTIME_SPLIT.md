...
6. **Engine robustness — TWO confirmed crash modes; THE #1 production blocker (bigger than Phase 0 first implied).** The single-worker engine hard-exits (no crash trace) under two conditions, both surfaced during Phase 0/1 validation:
   ...
   **Phase 4 status:** (i) ✅ FIXED (above) — the critical one; the engine now sustains a full session. **Remaining, all lower-priority now that sessions are stable:** extend the mode-(i) try/catch to the other 3 streaming providers (`/v1/revise`, `/v1/board`, `/intervene`) + fix `/intervene`'s dangling `&raw_vec`/`&applied_layers` captures (a confirmed separate stack-use-after-return, only on `/intervene` streaming); mode (ii) the mid-stream-disconnect guard (`cloze_server.cpp:1078` — capture `sink.write`'s bool + a cancel flag in `run()`'s loop; the studio consumes full streams so it doesn't hit this, but closed tabs / cancels do); (b) supervise the engine (auto-restart) + (c) studio retry-on-connection-refused as defense-in-depth. **With (i) fixed, the engine substrate is a usable *runtime* for ordinary sessions — not just a validated architecture.**
